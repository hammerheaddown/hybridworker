/*
 * hybridCC-stdin.exe â€” CEA-608 caption injector (stdin/stdout FLV pipe)
 *
 ...
 * text is used â€” injected into every video keyframe. No timestamp matching.
...
/* Current caption text â€” updated from VTT file */
...
static void reload_captions(void)
{
    ...
    /* Find the last text block â€” skip headers and timestamp lines */
    char* last_text = NULL;
    char* line = buf;
...
{
    if (argc < 2) {
        fprintf(stderr, "hybridCC-stdin â€” CEA-608 live caption injector (stdin/stdout FLV)\n\n");
    ...
    while (flv_read_tag(in, &tag)) {
        ...
        if (flvtag_avcpackettype_nalu == flvtag_avcpackettype(&tag)) {
            ...
            if (g_text[0] && !g_injected) {
                ...
                g_injected = 1; /* Only inject once per VTT reload â€” clear injects on next reload */
            } else if (g_text[0]) {
        ...
        fflush(out); /* Flush for pipe â€” don't buffer */
    }
